...
// 返回cat是否可用（是否被初始化）.
ERL_NIF_TERM isCatEnabled(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
...
// 记录Event.
ERL_NIF_TERM logEvent(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    char type[MAXKEYLEN];
...
// 记录错误Event.
ERL_NIF_TERM logError(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    char msg[MAXKEYLEN];
...
// 记录count类型Metric.
ERL_NIF_TERM logMetricForCount(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    char name[MAXKEYLEN];
    (void)memset(&name, '\0', sizeof(name));
...
// 记录duration类型Metric.
ERL_NIF_TERM logMetricForDuration(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    char name[MAXKEYLEN];
    (void)memset(&name, '\0', sizeof(name));
...
ERL_NIF_TERM logMetricForSum(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    char name[MAXKEYLEN];
    (void)memset(&name, '\0', sizeof(name));
...
// 记录耗时类的Transaction.
ERL_NIF_TERM logTransactionWithDuration(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    char type[MAXKEYLEN];
    (void)memset(&type, '\0', sizeof(type));
...
static ErlNifFunc nif_funcs[] = {
    ...
    {"is_cat_enabled", 0, isCatEnabled},
    {"destroy_cat", 0, destroyCatClient},

    {"log_event", 4, logEvent},
    {"log_error", 2, logError},
    {"log_metric_for_count", 2, logMetricForCount},
    {"log_metric_for_duration", 2, logMetricForDuration},
    {"log_metric_for_sum", 2, logMetricForSum},
    
    {"log_transaction_with_duration", 3, logTransactionWithDuration},
    
    {"new_transaction", 2, newTransaction},
